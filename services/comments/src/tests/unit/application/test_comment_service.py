...
def test_create_comment_success(comment_service, mock_repository, mock_publisher):
    ...
    # When
    inserted_id = comment_service.create_comment(comment_create_data, author_id, author_name)
    ...
    assert called_comment.author_id == author_id
    assert called_comment.author_name == author_name
...
def test_get_public_comments(comment_service, mock_repository):
    # Given
    mock_comments = [
        Comment(id="1", author_id="a1", author_name="n1", message="m1", is_public=True),
        Comment(id="2", author_id="a2", author_name="n2", message="m2", is_public=True),
    ]
    mock_repository.list_public.return_value = mock_comments

    # When
    comments = comment_service.get_public_comments()
